...
import { Estrellas } from "@/components/ProfesionalCard";
...
export default function PerfilProfesionalPage() {
  ...
  const cargarProfesional = async () => {
    try {
      // Cargar profesional
      const resProfesional = await fetch(
        `/api/profesionales?slug=${params.slug}`
      );
      if (!resProfesional.ok) throw new Error("No encontrado");
      const dataProfesional = await resProfesional.json();
      setProfesional(dataProfesional);

      // Cargar reseñas
      const resResenas = await fetch(
        `/api/resenas?profesional_id=${dataProfesional.id}`
      );
      if (resResenas.ok) {
  ...
  const getWhatsAppLink = () => {
    if (!profesional?.whatsapp) return null;
    const mensaje = encodeURIComponent(
      `Hola ${profesional.nombre}, te encontré en Famaillá Emprende y me gustaría consultar por tus servicios.`
    );
    return `https://wa.me/${profesional.whatsapp}?text=${mensaje}`;
  };
      ...
      <section className="bg-gradient-to-br from-indigo-600 via-indigo-500 to-purple-500 text-white py-12 relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6">
          ...
          <nav className="flex items-center gap-2 text-indigo-200 text-sm mb-6">
            ...
            <span className="text-white truncate">{profesional.nombre}</span>
          </nav>
          ...
          <div className="flex flex-col md:flex-row items-start gap-6">
            ...
            <div className="relative flex-shrink-0">
              {profesional.foto_url ? (
                ...
                <div className="w-32 h-32 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center text-white font-bold text-3xl shadow-xl border-4 border-white/20">
                  {iniciales}
                </div>
              )}
            ...
            <div className="flex-1">
              ...
              <h1 className="text-3xl font-bold mb-1">{profesional.nombre}</h1>
              <p className="text-indigo-200 text-lg mb-4">{profesional.profesion}</p>

              {/* Puntuación */}
              <div className="flex items-center gap-3">
                <Estrellas puntuacion={profesional.puntuacion_promedio || 0} size="large" />
                ...
                {mostrarFormResena && (
                  <form onSubmit={enviarResena} className="mb-6 bg-stone-50 rounded-xl p-5">
                    <h3 className="font-medium text-stone-800 mb-4">
                      Tu opinión sobre {profesional.nombre}
                    </h3>
                        ...
                        <div className="flex gap-2">
                          {[1, 2, 3, 4, 5].map((num) => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                setFormResena((prev) => ({ ...prev, puntuacion: num }))
                              }
                              className="focus:outline-none"
                            >
                              <svg
                                className={`w-8 h-8 ${
                                  num <= formResena.puntuacion
                                    ? "text-amber-400"
                                    : "text-stone-200"
              ...
              <div className="bg-white rounded-2xl border border-stone-200 p-6 sticky top-24">
                ...
                {profesional.instagram && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-medium py-3 rounded-xl transition-colors"
                  >
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                <div className="mt-6 pt-6 border-t border-stone-100 space-y-3">
                  ...
                  {profesional.direccion && (
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <span className="text-stone-600">{profesional.direccion}</span>
                    </div>
                  )}
                </div>
              </div>
