...
const categoriasFecha = {
  ...
  Capacitación: "from-indigo-400 to-indigo-500",
  Cultural: "from-pink-400 to-pink-500",
  Deportivo: "from-orange-400 to-orange-500",
  Social: "from-teal-400 to-teal-500",
};
...
export default function EventCard({ evento }) {
  ...
  return (
    <Link href={`/eventos/${evento.slug}`}>
      <article
        className={`group bg-white rounded-2xl border border-stone-200 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-100/50 hover:-translate-y-1 transition-all duration-300 overflow-hidden h-full ${
          yaPaso ? "opacity-60" : ""
        }`}
      >
        ...
        <div className="px-6 pb-6">
          ...
          {/* Botón ver más */}
          <div
            className={`w-full inline-flex items-center justify-center gap-2 text-sm font-semibold px-4 py-3 rounded-xl transition-all ${
              yaPaso
                ? "bg-stone-100 text-stone-500"
                : "bg-gradient-to-r from-emerald-500 to-teal-500 group-hover:from-emerald-600 group-hover:to-teal-600 text-white shadow-sm group-hover:shadow-md"
            }`}
          >
            {yaPaso ? (
